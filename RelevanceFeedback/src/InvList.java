...
public class InvList {
    ...
    /**
     * Inserts inverted list i2 into this inverted list (called "i1"). Maintains
     * a sorted order.
     */
	...
	while (i2Doc < i2.df) {
	    ...
	    // Skip documents in i1 that are less than the current document in
	    // i2.
	    while ((i1Doc < this.df)
		    && (this.postings.elementAt(i1Doc).docid < i2.postings
			    .elementAt(i2Doc).docid)) {
		i1Doc++;
	    }

	    // i1Doc points to where an i2 document can be merged or inserted
		...
		this.postings.elementAt(i1Doc).insert(
			i2.postings.elementAt(i2Doc));
