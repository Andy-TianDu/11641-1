...
public class DocPosting {
    ...
    /**
     * Inserts the postings from dp2 into this (dp1) docPosting. Maintains a
     * sorted order.
     */
	...
	while (dp2Pos < dp2.tf) {
	    ...
	    // Skip positions in dp1 that are less than the current position in
	    // dp2.
	    while ((dp1Pos < this.tf)
		    && (this.positions.elementAt(dp1Pos) < dp2.positions
			    .elementAt(dp2Pos))) {
		dp1Pos++;
	    }

	    // dp1Pos points to where a dp2 position can be inserted (possibly
