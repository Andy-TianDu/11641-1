...
public class Recommender {
    ...
    public static Strategy strategy = Strategy.user_based;
    
    public static void predictRating(Query query) {
	float prediction = 0;
	if (strategy == Strategy.user_based) {
	    User user = users.get(query.userId);
	    List<Neighbor> neighbors = user.findKNearestNeighibors(k, metric, useCache, useMean);
...
//		System.out.println(neighbor);
	    ...
	    List<Neighbor> neighbors = movie.findKNearestNeighibors(k, metric, useCache, useMean);
	    for (Neighbor neighbor : neighbors) {
...
//	System.out.println(prediction);
	...
	metric = (ISimilarityMetric)Class.forName(params.get("similarity")).newInstance();
	if (params.get("strategy").equals("user_based")) {
